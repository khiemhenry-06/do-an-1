...
def inverse(A):
    ...
    ma_tran_I = [[1.0 if i == j else 0.0 for j in range(so_cot)] for i in range(so_dong)]
    ...
    for col in range(so_cot):
        ...
        if abs(ma_tran_tang_cuong[max_row][col]) <= EPS:
            return None
        ...
        for row in range(col + 1, so_dong):
            if abs(ma_tran_tang_cuong[row][col]) > EPS:
                ...
                for c in range(2 * so_cot):
                    ...
                    if abs(ma_tran_tang_cuong[row][c]) <= EPS:
                        ma_tran_tang_cuong[row][c] = 0.0
